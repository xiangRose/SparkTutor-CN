...
class Step:
    ...
    # Optional learning-diagnosis metadata. Courses can adopt these gradually.
    knowledge_components: list[str] = field(default_factory=list)
    is_transfer: bool = False
    source_task: Optional[str] = None
    transfer_distance: Optional[str] = None
...
def load_lesson(lesson_dir: Path) -> Lesson:
    ...
    for raw in raw_steps:
        ...
        step = Step(
            ...
            knowledge_components=raw.get("KnowledgeComponents", []),
            is_transfer=raw.get("Transfer", False),
            source_task=raw.get("SourceTask"),
            transfer_distance=raw.get("TransferDistance"),
        )
        steps.append(step)
